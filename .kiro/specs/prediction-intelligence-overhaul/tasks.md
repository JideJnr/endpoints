...
- [x] 1. Add `ai_analysis_feedback`, `user_behavior_outcomes`, and `system_events` tables to `_init_learner_tables()`
...
- [x] 2. Implement `_incorporate_ai_analysis(conn, rows)` — replace stub
  - [x] 2.1 Write the full body of `_incorporate_ai_analysis(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 2.2 Write unit test for `_incorporate_ai_analysis` with no `competition_analysis` table
    - Confirm the function returns 0 and does not raise when the table is missing.
    - _Requirements: R1.7_

- [x] 3. Implement `_incorporate_user_behavior(conn, rows)` — replace stub
  - [x] 3.1 Write the full body of `_incorporate_user_behavior(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 3.2 Write unit test for `_incorporate_user_behavior` when no `user_pick_signal` rows exist
    - Confirm the function returns 0 and does not raise.
    - _Requirements: R2.8_

- [x] 4. Implement signal deduplication in `SignalAggregator` (R3)
  - [x] 4.1 Add `_dropped_duplicates: int = 0` instance attribute in `SignalAggregator.__init__()` in `app/enrichment/signal_aggregator.py`
    - _Requirements: R3.3_
  - [x] 4.2 Rewrite `add_signals()` to group incoming signals by `(category, source)` and deduplicate cross-source
  ...
  - [x] 4.3 Add `"dropped_duplicate_count": self._dropped_duplicates` to the dict returned by `calculate_probabilities()`
    - _Requirements: R3.4_
  - [x] 4.4 Write unit tests for deduplication behaviour
...
- [x] 5. Fix TTL cache for `_graded_rows()` in `learned_parameters.py` (R5)
  - [x] 5.1 Replace the `@lru_cache(maxsize=1)` decorator on `_graded_rows()` with a module-level time-aware cache in `app/monitoring/learned_parameters.py`
  ...
  - [x] 5.2 Update `clear_learned_parameter_cache()` to also reset `_GRADED_ROWS_CACHE = None` and `_GRADED_ROWS_FETCHED_AT = 0.0`
    - This forces an immediate re-fetch on the next call.
    - _Requirements: R5.3_
  - [x] 5.3 Write unit test for TTL re-fetch behaviour
...
- [x] 6. Fix cache invalidation placement in `run_learning_cycle()` (R5)
...
- [x] 7. Implement drift detection `_detect_and_handle_drift(conn, rows)` and wire into learning cycle (R4)
  - Prerequisite: tasks 1 (system_events table) and 6 (cache invalidation) must be complete before this task.
  - [x] 7.1 Write `_detect_and_handle_drift(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 7.2 Call `_detect_and_handle_drift(conn, rows)` in `run_learning_cycle()` after `update_tournament_preferences()` and add `"drift_events"` key to the return dict
    - _Requirements: R4.1, R4.6_
  - [x] 7.3 Write unit tests for drift detection
...
- [x] 9. Add `league_outcome_distribution` and `context_penalty_adjustments` tables to `_init_learner_tables()`
...
- [x] 10. Populate `league_outcome_distribution` in `run_learning_cycle()` (R6 step 5–6)
  - Prerequisite: task 9 (table must exist).
  - [x] 10.1 Write `_populate_league_outcome_distribution(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 10.2 Call `_populate_league_outcome_distribution(conn, rows)` as step 10 in `run_learning_cycle()` and add `"league_outcome_distribution_updates"` key to the return dict
    - _Requirements: R6.5_

- [x] 11. Replace hardcoded away baseline with `_get_away_baseline()` in `signal_aggregator.py` (R6)
  - Prerequisite: task 10 (table populated before prediction path reads it).
  - [x] 11.1 Write `_get_away_baseline(league_key: str) -> float` helper in `app/enrichment/signal_aggregator.py`
  ...
  - [x] 11.2 In `calculate_probabilities()`, replace the hardcoded `away_baseline = 0.54` with a call to `_get_away_baseline(self.league_key)`
    - _Requirements: R6.1, R6.2_
  - [x] 11.3 Write unit tests for `_get_away_baseline()`
...
- [x] 12. Replace hardcoded mixed-signal base probabilities with `_get_base_probs()` in `signal_aggregator.py` (R7)
  - Prerequisite: task 10 (table populated).
  - [x] 12.1 Write `_get_base_probs(league_key: str) -> tuple[float, float, float, str]` in `app/enrichment/signal_aggregator.py`
  ...
  - [x] 12.2 In `calculate_probabilities()` mixed-signal (`else`) branch, replace `home_prob = 0.45 + ...` / `away_prob = 0.25 + ...` / `draw_prob = 0.30 + ...` base constants with values from `_get_base_probs(self.league_key)`
    - Add `"base_probs_source"` key to the returned dict, always present.
    - _Requirements: R7.2, R7.4, R7.5_
  - [x] 12.3 Write unit tests for `_get_base_probs()`
...
- [x] 13. Implement `_learn_context_penalties()` and populate `context_penalty_adjustments` in the learning cycle (R8)
  - Prerequisite: task 9 (`context_penalty_adjustments` table created).
  - [x] 13.1 Write `_learn_context_penalties(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 13.2 Call `_learn_context_penalties(conn, rows)` inside `run_learning_cycle()` and add `"context_penalty_updates"` key to the return dict
    - _Requirements: R8.2_

- [x] 14. Wire learned context penalties into `contextual_intelligence.py` (R8)
  - Prerequisite: task 13 (table populated).
  - [x] 14.1 Write `_learned_penalty_for_tag(tag: str, league_key: str) -> float | None` helper in `app/enrichment/contextual_intelligence.py`
  ...
  - [x] 14.2 In `_match_context()`, before applying each hardcoded tag adjustment, call `_learned_penalty_for_tag(tag, league_key)` and use the returned override when not `None`
    - The `league_key` is derived from `doc.get("tournament")` or equivalent.
    - _Requirements: R8.4, R8.5, R8.6, R8.7_
  - [x] 14.3 Write unit tests for learned context penalty override
...
- [x] 15. Fix `_BASE_WEIGHTS` in `ensemble.py` and update `_get_weights()` fallback (R9)
  - This task is standalone — no prerequisites beyond the file itself.
  - [x] 15.1 Replace `_BASE_WEIGHTS: dict[str, float] = {}` in `app/models/ensemble.py` with:
    ```python
    ...
    ```
    - _Requirements: R9.1_
  - [x] 15.2 In `_get_weights()`, update the fallback path so that when `get_learned_weights()` returns an empty dict, the function returns `_BASE_WEIGHTS` instead of `{}`
  ...
  - [x] 15.3 Write unit tests for `_BASE_WEIGHTS` and `_get_weights()` fallback
...
- [x] 16. Fix direction-aware category fallback in `_category_for_signal()` (R10)
  - This task is standalone — no prerequisites.
  - [x] 16.1 Rewrite `_category_for_signal(name: str) -> str` in `app/enrichment/signal_aggregator.py`
  ...
  - [x] 16.2 Write unit tests for direction-aware category fallback
...
- [x] 18. Add `signal_outcomes` table to `_init_learner_tables()` and implement `_backfill_signal_outcomes()` (R12)
  - [x] 18.1 Add `signal_outcomes` table creation to `_init_learner_tables(conn)` in `app/monitoring/self_learner.py`
    ...
    - _Requirements: R12.4_n
  - [x] 18.2 Write `_backfill_signal_outcomes(conn, rows)` in `app/monitoring/self_learner.py`
  ...
  - [x] 18.3 Call `_backfill_signal_outcomes(conn, rows)` as the final step in `run_learning_cycle()` and add `"signal_outcome_backfills"` key to the return dict
    - _Requirements: R12.6_

- [x] 19. Inject team prediction history signals in `enriched_prediction.py` (R11)
  - [x] 19.1 Write helper `_team_history_signals(doc, detail) -> list[dict]` in `app/enrichment/enriched_prediction.py`
  ...
  - [x] 19.2 Call `_team_history_signals(doc, detail)` after `ensemble_prediction()` in `predict_enriched_match()` and append the returned signals to the prediction's signals list
    - _Requirements: R11.1_
  - [x] 19.3 Write unit tests for `_team_history_signals()`
...
- [x] 20. Implement H2H signal synthesis from SofaScore `last_meetings` (R13)
  - This task is standalone within `enriched_prediction.py` — no table prerequisites.
  - [x] 20.1 Write `_compute_h2h_signals(detail: dict) -> list[dict]` in `app/enrichment/enriched_prediction.py`
  ...
  - [x] 20.2 In `_rules_prediction(doc, detail)`, call `_compute_h2h_signals(detail)` and, before appending each signal, check if an existing signal with the same `name` and equal or higher `abs(impact)` already exists; skip the computed signal if so
    - _Requirements: R13.6_
  - [x] 20.3 Write unit tests for `_compute_h2h_signals()`
...
- [x] 21. Attach competition round analysis in `competition_special.py` and wire into `enriched_prediction.py` (R14)
  - [x] 21.1 In `apply_known_competition_context()` in `app/competition/competition_special.py`, after setting `doc["known_competition"]`, query `competition_analysis` for the most recent row matching the match's `competition_key`
  ...
  - [x] 21.2 In `_rules_prediction(doc, detail)` in `app/enrichment/enriched_prediction.py`, after existing signal computation, check `doc.get("competition_round_analysis")`
  ...
  - [x] 21.3 Write unit tests for competition round analysis wiring
...
- [x] 22. Add tournament priority confidence modifier to `SignalAggregator._calculate_confidence()` (R15)
  - [x] 22.1 At the end of `_calculate_confidence()` in `app/enrichment/signal_aggregator.py`, add a tournament priority modifier block
  ...
  - [x] 22.2 Write unit tests for tournament priority confidence modifier
...
- [x] 24. Add cache size guard to `prefetch_signal_stats()` and `global_signal_stats()` (R16)
...
- [x] 25. Normalise lineup detection across all key names (R17)
...
- [x] 26. Separate the 80–89% and 90–99% calibration bands in `confidence_calibrator.py` (R18)
  - In `app/enrichment/confidence_calibrator.py`:
  - [x] 26.1 In `rebuild_calibration()`, replace `min(80, (confidence / 10) * 10) as band_low` in both the per-pick_type query and the `'__global__'` query with:
    ```sql
    case
  ...
  - [x] 26.2 In `calibrate_confidence()`, replace `band_low = min(80, (raw_confidence // 10) * 10)` with:
    ```python
  ...
  - [x] 26.3 Write unit tests for calibration band separation
...
- [x] 31. Implement temporal decay weighting in `_tally()` (R19)
  - Prerequisite: Tier 1 core learning cycle must be complete.
  - [x] 31.1 Add a `_decay_weight(created_at: str, now: datetime) -> float` helper in `app/monitoring/self_learner.py`
  ...
  - [x] 31.2 Update `_tally()` (or the equivalent win-rate aggregation used for signal_weights, league_accuracy, and learned_model_weights) to use decay-weighted contributions
  ...
  - [x] 31.3 Write unit tests for temporal decay
...
- [x] 32. Implement confidence weighting in `_tally()` (R20)
  - Prerequisite: task 31 (temporal decay already in `_tally()`).
  - [x] 32.1 In `_tally()`, multiply each row's contribution by `confidence_weight = (row.get("confidence") or 50) / 100.0`
  ...
  - [x] 32.2 Verify the combined weighting is applied consistently across all three contexts (signal_weights, league_accuracy, learned_model_weights)
    - No separate tally paths should exist that bypass the combined weight.
    - _Requirements: R20.7_
  - [x] 32.3 Write unit tests for confidence weighting
...
- [x] 33. Make league adjustment caps symmetric in `enriched_prediction.py` (R21)
  - This task is standalone — no prerequisites.
  - [x] 33.1 In `app/enrichment/enriched_prediction.py`, locate the league accuracy penalty/boost application block (around lines 766–773)
  ...
  - [x] 33.2 Write unit tests for symmetric caps
...
- [x] 34. Replace hardcoded bias correction floor with dynamic formula (R22)
  - This task is standalone within `self_learner.py`.
  - [x] 34.1 In `app/monitoring/self_learner.py`, locate the bias correction multiplier floor (around line 947)
  ...
  - [x] 34.2 Write unit tests for the dynamic bias floor
...
- [x] 35. Raise signal combination memory sample guard to 12 (R23)
  - This task is standalone within `self_learner.py`.
  - [x] 35.1 Add `MIN_COMBINATION_SAMPLES = 12` as a named constant at module level in `app/monitoring/self_learner.py`
  ...
  - [x] 35.2 In `_learn_signal_combinations()`, replace the existing numeric threshold (5) with `MIN_COMBINATION_SAMPLES`
  ...
  - [x] 35.3 Write unit tests for the raised sample guard
